...
export async function POST(request: Request) {
  ...
  let body: { round_id: string; result: string }
  try {
  ...
  const { round_id, result } = body
  if (!round_id || !result) return fail('round_id and result are required')

  const db = createAdminClient()

  // Call settle_casino_round RPC
  const { data, error } = await db.rpc('settle_casino_round', {
    p_round_id: round_id,
    p_result: result,
