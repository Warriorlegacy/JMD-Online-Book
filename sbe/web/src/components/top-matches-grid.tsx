        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {matches.map((match) => (
            <MatchCard 
              ...
              match={{
                id: match.id,
                teamA: String(match.teams?.[0] || ''),
                teamB: String(match.teams?.[1] || ''),
                startTime: match.time,
                status: match.isLive ? "in_play" : "scheduled",
                sportType: "Football",
                score: match.score ? { teamA: String(match.score?.[0] ?? '').toString(), teamB: String(match.score?.[1] ?? '').toString() } : undefined,
                league: match.league,
                odds: [
                ...
                <div className="col-span-5 flex items-start gap-5">
                  ...
                  <div className="flex flex-col gap-1">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-semibold text-white tracking-tight">{String(match.teams?.[0] || '')}</span>
                      {match.isLive && match.score && (
                        <span className="text-[13px] font-bold text-[#ff3b30] tabular-nums ml-auto">{String(match.score?.[0] ?? '')}</span>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-semibold text-white tracking-tight">{String(match.teams?.[1] || '')}</span>
                      {match.isLive && match.score && (
                        <span className="text-[13px] font-bold text-[#ff3b30] tabular-nums ml-auto">{String(match.score?.[1] ?? '')}</span>
                      )}
                    </div>
                  </div>
                </div>
