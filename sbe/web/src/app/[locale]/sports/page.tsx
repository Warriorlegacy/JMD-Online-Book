    ...
    <div className="relative flex-shrink-0 w-72 glass-card rounded-2xl border border-white/5 p-5 overflow-hidden">
      ...
      <div className="flex items-center justify-between mb-4">
        <span className="px-2 py-1 rounded-lg bg-[#0071e3]/15 border border-[#0071e3]/20 text-[#0071e3] text-[8px] font-black uppercase tracking-widest">
          {m.tournamentName} • {m.elapsedMinutes ? `${m.elapsedMinutes}'` : 'LIVE'}
        </span>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-7">
          ...
          <div>
            ...
            <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
              {matches.filter(m => m.status === "in_play").length > 0 ? matches.filter(m => m.status === "in_play").map(m => <LiveMatchCard key={m.id} m={m} />) : <div className="text-white/50 text-sm py-4 px-2">No live matches available.</div>}
            </div>
          </div>
                  ...
                  {matches.filter(m => m.status === "in_play" || m.status === "scheduled").slice(0, 3).map(m => (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-center gap-1.5">
                        <OddsBtn value={m.odds?.['teamA']?.[0]?.price ?? "—"} onClick={() => quickSelect(m.teamA, m.teamB, m.id, m.teamA, m.odds?.['teamA']?.[0]?.price ? parseFloat(m.odds?.['teamA']?.[0]?.price) : 0)} blue />
                        <OddsBtn value={m.odds?.['draw']?.[0]?.price ?? "—"} onClick={() => quickSelect(m.teamA, m.teamB, m.id, "Draw", m.odds?.['draw']?.[0]?.price ? parseFloat(m.odds?.['draw']?.[0]?.price) : 0)} />
                        <OddsBtn value={m.odds?.['teamB']?.[0]?.price ?? "—"} onClick={() => quickSelect(m.teamA, m.teamB, m.id, m.teamB, m.odds?.['teamB']?.[0]?.price ? parseFloat(m.odds?.['teamB']?.[0]?.price) : 0)} />
                      </div>
                      <div className="flex items-center justify-center gap-1.5">
