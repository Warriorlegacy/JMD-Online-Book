...
function LiveMatchCard({ m }: { m: any }) {
  return (
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-7">
          ...
          <div>
            ...
            <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
              {matches.filter(m => m.status === "in_play").map((m: any) => <LiveMatchCard key={m.id} m={{
                  id: m.id, league: m.tournamentName || "LIVE EVENT", time: m.elapsedMinutes ? `${m.elapsedMinutes}'` : "LIVE",
                  teamA: m.teamA, teamB: m.teamB, scoreA: m.score?.teamA || 0, scoreB: m.score?.teamB || 0,
                  status: "in_play", live: true, oddsA: 1.85, oddsD: 3.20, oddsB: 4.15, extraLabel: "LIVE"
                }} />)}
            </div>
          </div>
